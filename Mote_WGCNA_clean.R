...
#file
save(datExpr, datTraits,datInteract, file="MoteSamplesAndTraitsAndInteractionssigned_Feb23.RData")
...
save(adjacency, TOM, dissTOM, file="MoteSamplesAndTraits_TOM_signed_Jun22.RData")
...
save(MEs, moduleLabels, moduleColors, geneTree, file= "MoteNetwork_rlog_signed_unmerged_sft14_Feb23.RData")
...
lnames = load(file = "MoteSamplesAndTraitsAndInteractionssigned_Feb23.RData") 
#The variable lnames contains the names of loaded variables

# Load network data saved in the second part. Change this based on the level of merge you want to work with
lnames = load(file = "MoteNetwork_rlog_signed_unmerged_sft14_Feb23.RData")
...
MEs.sorted <- MEs%>%
  ...
  arrange(treatment)%>%
  select(MEred, MEmagenta, MEcyan, MEyellow, MEpurple, MEmidnightblue, treatment, genotype)%>%
...
#test whether combined effect is significantly different from the added effects of heat and pH
medians <- MEs.sorted%>%
  gather(module, ME, -treatment, -genotype)%>%
  mutate(treatment = recode (treatment, "Heat" = "combinedExpect", "pH" = "combinedExpect"))%>%
  group_by(treatment, module)%>%
  dplyr::summarise(median = median(ME, na.rm=TRUE), .groups="keep")%>%
  spread(treatment, median)%>%
  mutate(expected =(combinedExpect - Control))

statsyellow <- MEs.sorted%>%
  gather(module, ME, -treatment, -genotype)%>%
  filter(module == "MEyellow", treatment == "Combined")%>%
  mutate(standardized_ME = ME - 0.07555845)

wilcox.test(statsyellow$standardized_ME, mu = -0.07443855, alternative = "less")


#V = 22, p-value = 9.521e-13
#alternative hypothesis: true location is less than -0.07443855

statspurple <- MEs.sorted%>%
  gather(module, ME, -treatment, -genotype)%>%
  filter(module == "MEpurple", treatment == "Combined")%>%
  mutate(standardized_ME = ME - -0.07439893)

wilcox.test(statspurple$standardized_ME, mu = 0.07027208, alternative = "greater")

#V = 1222, p-value = 8.882e-15
#alternative hypothesis: true location is greater than 0.07027208

statsmidnightblue <- MEs.sorted%>%
  gather(module, ME, -treatment, -genotype)%>%
  filter(module == "MEmidnightblue", treatment == "Combined")%>%
  mutate(standardized_ME = ME -0.06928525)

wilcox.test(statsmidnightblue$standardized_ME, mu = 0.07491188, alternative = "greater")

#V = 0, p-value = 1
#alternative hypothesis: true location is greater than 0.07491188

statscyan <- MEs.sorted%>%
  gather(module, ME, -treatment, -genotype)%>%
  filter(module == "MEcyan", treatment == "Combined")%>%
  mutate(standardized_ME = ME --	0.06751502)

wilcox.test(statscyan$standardized_ME, mu = -0.07862293, alternative = "less")

#V = 1185, p-value = 1
#alternative hypothesis: true location is less than -0.07862293

statsred <- MEs.sorted%>%
  gather(module, ME, -treatment, -genotype)%>%
  filter(module == "MEred", treatment == "Combined")%>%
  mutate(standardized_ME = ME - 0.03785620)

wilcox.test(statsred$standardized_ME, mu = 0.04936407, alternative = "greater")

#V = 1, p-value = 1
#alternative hypothesis: true location is greater than 0.04936407

statsmagenta<- MEs.sorted%>%
  gather(module, ME, -treatment, -genotype)%>%
  filter(module == "MEmagenta", treatment == "Combined")%>%
  mutate(standardized_ME = ME -	0.06039775)

wilcox.test(statsmagenta$standardized_ME, mu = -0.06317803, alternative = "less")

#V = 37, p-value = 1.019e-11
#alternative hypothesis: true location is less than -0.06317803
